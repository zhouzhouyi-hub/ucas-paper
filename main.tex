...
\begin{tikzpicture}[node distance=1.0cm, auto]
  ...
  \node[block, below left=1.0cm and 1.4cm of cond] (force) {Force quiescent state};
  \node[block, below right=1.0cm and 1.4cm of cond] (normal) {Continue normally};
  \node[block, below=2.6cm of cond] (end) {Finish};
  ...
  \draw[arrow] (force.south) -- ++(0,-0.4) |- (end.west);
  \draw[arrow] (normal.south) -- ++(0,-0.4) |- (end.east);
\end{tikzpicture}%
}
...
\begin{tikzpicture}[node distance=0.9cm, auto]
  % Styles (use \tikzset instead of deprecated \tikzstyle)
  \tikzset{
    block/.style={
...
\begin{tikzpicture}[
  ...
  decision/.style={
    diamond, draw, aspect=1.4, align=center,
...
\node[decision, below=of get]   (cond) {No timer?};
\node[block,    below=of cond]  (proc) {Remove \\ Mark \\ Cancel};
\node[block,    below=of proc]  (finish) {Finish};

% Connections
\draw[arrow] (start) -- (get);
\draw[arrow] (get) -- (cond);
\draw[arrow] (cond.south) -- node[right, xshift=2pt] {No} (proc.north);
\draw[arrow] (cond.east) -- ++(1.0,0) node[above, pos=0.55] {Yes} |- (finish.east);
\draw[arrow] (proc.west) -- ++(-1.0,0) |- (cond.west);
...
illustrates the shortest (left) and longest (right) GPIO response times
through the isolator. The green trace consistently departs from the
x-axis at approximately 10~us, corresponding to the oscilloscope
trigger on channel~1. The red trace departs from the x-axis at times
...
\begin{tabular}{lccc}
\hline
\textbf{Configuration} & \textbf{Period} & \textbf{Min Lat} & \textbf{Max Lat} \\
\hline
With Isolator     & 2000~ns   & 390~ns   & 470~ns  \\
PREEMPT-RT Linux  & 200000~ns & 4100~ns  & 72000~ns \\
\hline
\end{tabular}
}
...
\textbf{Configuration} & \textbf{Period} & \textbf{Min Lat} & \textbf{Max Lat} \\
\hline
With Isolator     & 2000~ns   & 0~ns   & 40~ns \\
PREEMPT-RT Linux  & 200000~ns & 0~ns   & 8000~ns \\
\hline
\end{tabular}
}
...
\textbf{Configuration} & \textbf{Period} & \textbf{Min Lat} & \textbf{Max Lat} \\
\hline
With Isolator     & 2000~ns   & 120~ns   & 160~ns \\
PREEMPT-RT Linux  & 200000~ns & 1040~ns  & 2200~ns \\
\hline
\end{tabular}
}
