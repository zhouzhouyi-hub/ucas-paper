...
dengke.du, mengke.sun\}@ucas.com.cn\\ 
}
...
\subsection{Inter-core Ping-Pong Test}

To further evaluate the determinism of the isolation framework in multicore environments, we
conducted an inter-core ping-pong experiment. In this test, two application threads pinned to
separate dedicated cores exchanged minimal messages through a shared-memory ring buffer.
Each message transfer was accompanied by an explicit invocation of the Isolator API to trigger
deferred maintenance tasks, thereby assessing the impact of cross-core interaction on latency
stability.
...
\caption{Latency comparison under different configurations (Inter-core Ping-Pong).}
\label{tab:latency-comparison3}
\end{table}

Results show that with the Isolator enabled, round-trip latencies consistently remained in the
sub-microsecond range, with a minimum of about 120~ns, a maximum below 160~ns, and jitter
under 40~ns Figure~\ref{fig:islcrossgpio}. In contrast, under PREEMPT-RT Linux, round-trip latency exhibited significant
variability, with maximum values exceeding 2~µs Figure~\ref{fig:linuxcrossgpio}. These findings further demonstrate that the
isolation framework effectively eliminates involuntary interruptions and ensures low-jitter,
deterministic execution even in cross-core communication scenarios as shown in Table~\ref{tab:latency-comparison3}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/islcrosscoreintgpio.png}
  \caption{Oscilloscope persistence waveform showing shortest (left) and longest (right) GPIO response times (isolator cross-core ping-pong)}
...
\begin{figure}[h]
  ...
  \caption{Oscilloscope persistence waveform showing shortest (left) and longest (right) GPIO response times (PREEMPT-RT cross-core ping-pong)}
